...
| ---- | --------- | ------------ | ---- | ----------- |
| Local web | `local` | `web` (default) | None — auto session | `go run .` + Vite dev |
| Desktop | `local` | `desktop` | None | Tauri app |
| Docker | `docker` | unset / `web` | Optional `APP_AUTH_TOKEN` | Single container |
| Server web | `docker` or `local` | `web` | **`APP_AUTH_TOKEN` required** | Reverse-proxy deployment |

When `APP_AUTH_TOKEN` is set, clients must exchange the token via `POST /api/config/auth` before API access. Token must be ≥ 32 characters or the process exits at startup.
...
| `APP_CLIENT` | No | — | `desktop` or `web` — desktop skips some web-only gates |
...
| `APP_AUTH_TOKEN` | No | — | Server-mode shared secret (≥ 32 chars) |
| `APP_ALLOWED_ORIGINS` | No | — | Comma-separated extra CORS origins (server-web) |
...
- **Local-web / desktop** (`APP_AUTH_TOKEN` unset): localhost origins are allowed for credentialed requests.
- **Server-web** (`APP_AUTH_TOKEN` set): only `APP_ALLOWED_ORIGINS` plus the app's own origin — localhost is **not** automatically trusted.
...
| `APP_AUTH_TOKEN` | unset | Required for server-web (≥ 32 chars) |
| `APP_ALLOWED_ORIGINS` | unset | Comma-separated trusted origins |

Example override at deploy time:

```bash
APP_PUBLIC_URL=https://dbo.example.com \
APP_AUTH_TOKEN='your-long-random-secret-at-least-32-chars' \
...
| `APP_MCP_PUBLIC_URL` | unset | Optional MCP URL override |
| `APP_AUTH_TOKEN` | unset | Optional server-mode auth |
| `APP_ALLOWED_ORIGINS` | unset | Optional CORS origins |
