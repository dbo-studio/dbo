...
// OptionalOrKeep returns value when it is present, including explicit empty strings.
// Use for PATCH fields that can be cleared by sending "".
func OptionalOrKeep[T any](value *T, defaultValue *T) *T {
	if value != nil {
		return value
	}
