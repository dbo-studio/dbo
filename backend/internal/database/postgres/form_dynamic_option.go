...
func (r *PostgresRepository) fkValueOptions(ctx context.Context, node contract.DBNode, params map[string]string) ([]contract.FormFieldOption, error) {
	...
	columns, err := r.columns(ctx, &node.Database, &targetTable, &schema, []string{}, false, true)
	if err != nil {
		return nil, err
	}
	...
	fromClause := databaseCore.QuotePGIdent(targetTable)
	if schema != "" {
		fromClause = databaseCore.QuotePGIdent(schema) + "." + fromClause
	}

	query := databaseCore.BuildFkLookupQuery(databaseCore.FkLookupDialect{
		QuoteIdent:   databaseCore.QuotePGIdent,
		CastToText:   func(quoted string) string { return quoted + "::text" },
		LikeOperator: "ILIKE",
	...
	sqlRows, err := conn.WithContext(lookupCtx).Raw(query.SQL, query.Args...).Rows()
	if err != nil {
		return nil, err
	}
	defer sqlRows.Close()
