...
func (r *MySQLRepository) handleViewCommands(node MySQLNode, tabId contract.TreeTab, action contract.TreeNodeActionName, data []byte) ([]string, error) {
	queries := []string{}
	...
	if action == contract.CreateViewAction {
		...
		if params.New.Name != nil && params.New.Query != nil {
			query = fmt.Sprintf("CREATE VIEW `%s`.`%s` AS %s", node.Database, *params.New.Name, *params.New.Query)
		}
