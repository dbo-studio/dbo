...
DROP TABLE IF EXISTS ${table1};
  ...
  test("Unqualified select uses editor database when connection has no default", async ({
    page,
  }, testInfo) => {
    const suffix = uniqueTestSuffix(testInfo);
    const connectionName = `crud-mysql-use-db-${suffix}`;
    const tableName = `e2e_use_db_${suffix}`;
    const config = {
    ...
    await withConnectionCleanup(page, connectionName, async () => {
      ...
      await test.step("Create MySQL connection without a default database", async () => {
        await connectionPage.setupConnection(config);
        await expect(
      ...
      try {
        await test.step("Select editor database and run unqualified SELECT", async () => {
          await sqlEditor.open();
          await sqlEditor.selectContext("default");
          await sqlEditor.typeAndRun(
            `
...
INSERT INTO ${tableName} (name) VALUES ('addon-row');
SELECT name FROM ${tableName};
            `.trim(),
          );
          await dataGrid.waitForData("addon-row");
          await expect(
            page.getByText(/No database selected/i),
          ).toHaveCount(0);
        });
      } finally {
