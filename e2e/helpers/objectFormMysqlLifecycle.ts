...
export async function setupMysqlConnection(
  page: Page,
  ...
  await expect
    .poll(
      async () =>
        ...
        (await objectForm
          .getGeneralField(F.databaseName)
          .locator("input")
          .isVisible()),
      { timeout: 60000 },
    )
...
export async function createPostsTable(
  ...
  await objectForm.selectMultiSelectOptions(createKeyRowIndex, F.keyColumns, [
    "id",
  ]);
  await objectForm.selectArrayCellOption(
    createKeyRowIndex,
  ...
  const dynamicOptionsPromise = page.waitForResponse(
    (response) =>
      response.url().includes("/dynamic") && response.status() === 200,
    { timeout: 15000 },
  );
  await objectForm.selectArrayCellOption(
    fkRowIndex,
  ...
  await objectForm.fillArrayCell(
    indexRowIndex,
    F.indexName,
    constraintNames.indexName,
  );
  await objectForm.selectMultiSelectOptions(indexRowIndex, F.indexColumns, [
    "user_id",
  ]);
  await objectForm.save();
...
export async function createView(
  ...
  await objectForm.fillGeneralQueryField(
    F.viewQuery,
    `SELECT id, user_id FROM ${postsTable}`,
  );
...
export async function editViewQuery(
  ...
  await objectForm.fillGeneralQueryField(
    F.viewQuery,
    `SELECT COUNT(*) AS total FROM ${postsTable}`,
  );
...
export async function cleanupMysqlLifecycle(
  ...
  await expect(tree.getTreeNode(names.postsTable)).toBeHidden({
    timeout: 15000,
  });

  await tree.dropObject(names.usersTable, "Drop table");
  await expect(tree.getTreeNode(names.usersTable)).toBeHidden({
    timeout: 15000,
  });
  ...
  if (options?.dropDatabase !== false) {
    ...
    await expect(tree.getTreeNode(names.databaseName)).toBeHidden({
      timeout: 15000,
    });
  }
