...
export default function EditColumnDialog({ open, onClose, nodeId, columnName, onSuccess }: EditColumnDialogProps) {
  const {
    tabs,
    selectedTabId,
    fields,
  ...
  const columnField = useMemo(() => {
    ...
    const columnIndex = columns.findIndex((col: any) => col.column_name === columnName || col.name === columnName);
